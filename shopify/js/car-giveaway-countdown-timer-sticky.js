(function () {
  ...
  function initCountdown() {
    ...
    var root = document.querySelector('.countdown-sticky');
    if (!root) return;

    var from = root.getAttribute('data-from');

    var defaultEnd = moment().tz("America/Los_Angeles")
      .add(customDays_sticky, 'days')
      .add(11, 'hours')
      .add(59, 'minutes')
      .add(59, 'seconds');

    var endTime = from
      ? moment.tz(from, "America/Los_Angeles").add(customDays_sticky, 'days').add(11, 'hours').add(59, 'minutes').add(59, 'seconds')
      : defaultEnd;
